...
class OrdersPerMonthChart extends ChartWidget
{
    ...
    protected function getData(): array
    {
        ...
        // =========================
        if ($demo) {
            [$labels, $values] = $this->demoCountPerMonth(12);
    ...
    /**
     * ✅ داتا وهمية: Orders per month (12 شهر) بشكل طبيعي + موسمية بسيطة
     */
    private function demoCountPerMonth(int $months = 12): array
    {
        $labels = [];
        $values = [];
        ...
        // “موسمية” بسيطة خلال السنة (ترفع وتنزل)
        $season = [0, 20, 35, 55, 40, 70, 95, 80, 60, 45, 30, 50];
        ...
        for ($i = 0; $i < $months; $i++) {
            ...
            // نمو بسيط عبر الوقت
            $trend = (int) round($i * 12);

            // ضوضاء خفيفة
            $noise = random_int(-35, 35);

            $v = $base + $trend + ($season[$i] ?? 0) + $noise;
